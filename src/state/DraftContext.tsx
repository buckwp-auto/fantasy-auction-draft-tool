...
import type { AppState, LeagueSettings, Pick, Player } from '../types'
...
export function DraftProvider({ children }: { children: ReactNode }) {
  ...
  const setPlayers = useCallback((players: Player[], mode: 'replace' | 'merge') => {
    setState((s) => {
      ...
      for (const p of players) {
        ...
        map.set(p.id, existing ? { ...p, target: existing.target } : p)
      }
  ...
  const toggleTarget = useCallback((playerId: string) => {
    setState((s) => ({
      ...s,
      players: s.players.map((p) =>
        p.id === playerId ? { ...p, target: !p.target } : p,
      ),
    }))
  }, [])
